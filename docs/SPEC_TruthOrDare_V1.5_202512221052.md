...
版本: V1.5_202512221052  
更新时间(UTC+8): 2025-12-22 10:52:00 CST

## 版本记录
| 版本 | 日期 | 更新内容 |
|------|------|---------|
| V1.5 | 2025-12-22 | **功能增强**：实现模式区分话题维度，每种风格在真心话/大冒险模式下有不同的话题维度 |
...
│   ├── prompt/
│   │   ├── builder.js        # ✅ Prompt构建逻辑（V1.5增强：支持模式区分维度）
│   │   └── dimensions.js     # ✅ 话题维度定义（V1.5增强：模式区分结构）
│   └── config/
...
- **真心话模式**：
  ...
  - **模式要求**：必须是提问、询问、要求回答；题目开头：你...、描述...、讲述...、说说...、回忆...
  - **绝对禁止**：任何需要"做动作"的内容（如：模仿、表演、做出...动作）
- **大冒险模式**：
  - 你是派对互动策划助手。根据模式与风格生成简洁、可执行的问题或任务，避免不当内容。
  - **模式要求**：必须是命令、指令、要求执行；题目开头：模仿...、做出...、表演...、完成...、向...做...
  - **绝对禁止**：任何只需要"说话回答"的内容

### 用户模板（增强版）
- 语言：`{{locale}}`；模式：`{{mode}}`；风格：`{{style}}`；数量：`{{count}}`
- 受众年龄：`{{audienceAge}}`；尺度：`{{intensity}}`；题目编号：`{{seed}}`
- **🎯 核心话题维度**：`{{dimension}}`（V1.5新增，根据模式和风格动态选择）
